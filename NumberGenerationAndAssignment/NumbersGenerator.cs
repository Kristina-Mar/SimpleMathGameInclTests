namespace NumberGenerationAndAssignment
{
    public class NumbersGenerator
    {
        ...
        {
            int number1Max = NumberLimit; // The maximum is not included in the random number generator range. Also sets the maximum result.
        ...
        public AssignedNumbersAndResult GenerateMultiplicationProblem()
        {
            ...
            // If the game only goes up to 100, the maximum number for multiplication is 50, ie. range up to 51 (not included).
            int number1 = randomNumberGenerator.Next(2, number1Max); // Multiplication by 1 is too easy.
            int number2Max = (NumberLimit / number1) + 1; // This way, the result will be lower or equal to the highest possible number.
            int number2 = randomNumberGenerator.Next(2, number2Max);
            int result = number1 * number2;
            return new AssignedNumbersAndResult(number1, number2, result);
        }
        public AssignedNumbersAndResult GenerateDivisionProblem()
        {
            int number1Max = (NumberLimit / 2) + 1; // If the game only goes up to 100, the maximum number for multiplication is 50, ie. range up to 51 (not included).
            int number1 = randomNumberGenerator.Next(2, number1Max); // Division by 1 is too easy.
            int number2Max = (NumberLimit / number1) + 1; // This way, the result will be lower or equal to the maximum number (scope of the problem).
            int number2 = randomNumberGenerator.Next(2, number2Max);
            int result = number1 * number2;
            ...
            // The switch ensures randomised order.
            if (number1 == 2)
            {
